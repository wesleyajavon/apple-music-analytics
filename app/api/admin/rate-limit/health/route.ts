...
function computeHealthStatus(params: {
  ...
  if (params.maxBlockedInCurrentMinute >= params.criticalThreshold) return "critical";
  if (params.maxBlockedInCurrentMinute >= params.warnThreshold) return "warn";
  return "ok";
}
...
export async function GET(request: NextRequest) {
  try {
    ...
    const criticalThreshold = parsePositiveInt(
      request.nextUrl.searchParams.get("criticalThreshold"),
      Number.parseInt(process.env.RATE_LIMIT_SPIKE_ALERT_THRESHOLD ?? "20", 10) || 20,
      1,
      10_000
    );
    const warnThreshold = parsePositiveInt(
      request.nextUrl.searchParams.get("warnThreshold"),
      Math.max(1, Math.floor(criticalThreshold * 0.5)),
      1,
      criticalThreshold
    );
    ...
    const perRoute = await Promise.all(
      routes.map(async (route) => {
        const blockedInCurrentMinute = await getRateLimitBlockedInCurrentMinute(route);
        return { route, blockedInCurrentMinute };
      })
    );

    perRoute.sort((a, b) => b.blockedInCurrentMinute - a.blockedInCurrentMinute);
    const maxBlockedInCurrentMinute = perRoute[0]?.blockedInCurrentMinute ?? 0;
    const totalBlockedInCurrentMinute = perRoute.reduce(
    ...
    return NextResponse.json({
      ...
      hottestRoute: perRoute[0] ?? null,
      generatedAt: new Date().toISOString(),
    });
  } catch (error) {
    return handleApiError(error, { route: "/api/admin/rate-limit/health" });
  }
}
