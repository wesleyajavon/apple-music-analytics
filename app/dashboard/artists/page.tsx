        ...
        <div className="flex items-start gap-4">
          ...
          <div className="min-w-0 flex-1">
            <p className="text-xs font-medium uppercase tracking-wider text-gray-500 dark:text-gray-400">
              {rankLabel} · {artist.listenCount.toLocaleString("fr-FR")} écoutes
            </p>
            <h3 className="mt-0.5 truncate text-lg font-bold text-gray-900 dark:text-white">
              {artist.artistName}
            </h3>
            <p className="text-sm text-gray-500 dark:text-gray-400">
              {artist.uniqueTracks} titre{artist.uniqueTracks > 1 ? "s" : ""} unique
              {artist.uniqueTracks > 1 ? "s" : ""}
            </p>
          ...
          >
            ...
            <div className="relative p-6">
              <div className="flex flex-col items-center text-center">
                ...
                <p className="text-sm text-gray-500 dark:text-gray-400">écoutes</p>
...
function ArtistsContent() {
  ...
  const trendChartData = useMemo(() => {
    ...
    return Array.from(dateMap.entries()).map(([date, artists]) => {
      ...
      return {
        date,
        formattedDate: d.toLocaleDateString("fr-FR", { day: "2-digit", month: "2-digit" }),
        ...artists,
      };
    });
  }, [trendsData]);
  ...
  if (error) {
    return (
      <ErrorState
        error={error}
        message="Impossible de charger les statistiques des artistes"
        onRetry={refetch}
      />
    );
  }
  if (!data || data.topArtists.length === 0) {
    return (
      <EmptyState
        ...
        message="Aucune donnée d'artiste disponible"
        description="Importez vos données Last.fm ou Apple Music Replay pour voir vos artistes."
      />
    );
  }
    ...
    <div className="space-y-8">
      ...
      <div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-violet-600 via-purple-600 to-fuchsia-600 px-6 py-8 shadow-2xl sm:px-8 sm:py-10">
        ...
        <div className="relative">
          <h2 className="text-2xl font-bold text-white sm:text-3xl">Vos artistes</h2>
          <p className="mt-1 text-white/90">Vue d&apos;ensemble de votre écoute</p>
          <div className="mt-6 flex flex-wrap gap-4 sm:gap-8">
            <div className="rounded-xl bg-white/15 px-4 py-3 backdrop-blur-sm">
              <p className="text-xs font-medium uppercase tracking-wider text-white/80">Artistes</p>
              <p className="text-2xl font-bold text-white">{overview.totalArtists.toLocaleString("fr-FR")}</p>
            </div>
            <div className="rounded-xl bg-white/15 px-4 py-3 backdrop-blur-sm">
              <p className="text-xs font-medium uppercase tracking-wider text-white/80">Écoutes</p>
              <p className="text-2xl font-bold text-white">{overview.totalListens.toLocaleString("fr-FR")}</p>
            </div>
            <div className="rounded-xl bg-white/15 px-4 py-3 backdrop-blur-sm">
              <p className="text-xs font-medium uppercase tracking-wider text-white/80">Top artiste</p>
              <p className="text-2xl font-bold text-white">{overview.topArtistListenCount.toLocaleString("fr-FR")}</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <section>
        <h3 className="mb-4 text-lg font-semibold text-gray-900 dark:text-white">Vos 3 artistes les plus écoutés</h3>
        <TopThreeArtists artists={topArtists} maxListens={maxListens} />
      </section>
      ...
      {trendChartData.length > 0 && (
        <div className="overflow-hidden rounded-2xl border border-gray-200/80 dark:border-gray-600/50 bg-white dark:bg-gray-800/90 shadow-lg">
          <div className="border-b border-gray-100 dark:border-gray-700/50 px-6 py-4">
            <h3 className="text-lg font-semibold text-gray-900 dark:text-white">Évolution des top artistes</h3>
            <p className="text-sm text-gray-500 dark:text-gray-400 mt-0.5">Écoutes par jour · 5 artistes</p>
          </div>
          <div className="p-6">
      ...
      <section>
        <h3 className="mb-4 text-lg font-semibold text-gray-900 dark:text-white">Tous vos artistes</h3>
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {topArtists.map((artist, index) => (
            <ArtistCard key={artist.artistId} artist={artist} rank={index + 1} maxListens={maxListens} />
          ))}
        </div>
      </section>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
        <div className="overflow-hidden rounded-2xl border border-gray-200/80 dark:border-gray-600/50 bg-white dark:bg-gray-800/90 shadow-lg">
          <div className="border-b border-gray-100 dark:border-gray-700/50 px-6 py-4">
            <h3 className="text-lg font-semibold text-gray-900 dark:text-white">Top 10 · Écoutes</h3>
          </div>
          <div className="p-6">
              ...
              <BarChart data={barChartData} layout="vertical" margin={{ top: 5, right: 30, left: 100, bottom: 5 }}>
                ...
                <Tooltip
                  ...
                  formatter={(value: number, name: string, props: { payload?: { fullName?: string } }) => {
                    ...
                    if (name === "écoutes") return [`${value.toLocaleString("fr-FR")} écoutes`, fullName || "Artiste"];
                    return [value, name];
                  }}
                />
                <Bar dataKey="écoutes" fill="url(#barGradient)" radius={[0, 6, 6, 0]} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </div>
        ...
        <div className="overflow-hidden rounded-2xl border border-gray-200/80 dark:border-gray-600/50 bg-white dark:bg-gray-800/90 shadow-lg">
          <div className="border-b border-gray-100 dark:border-gray-700/50 px-6 py-4">
            <h3 className="text-lg font-semibold text-gray-900 dark:text-white">Répartition · Top 6</h3>
          </div>
          <div className="p-6">
            <ResponsiveContainer width="100%" height={360}>
              <PieChart>
                ...
                <Tooltip
                  ...
                  labelStyle={CHART_TOOLTIP_STYLES.labelStyle}
                  itemStyle={CHART_TOOLTIP_STYLES.itemStyle}
                  formatter={(value: number) => [`${value.toLocaleString("fr-FR")} écoutes`, "Écoutes"]}
                />
              </PieChart>
            </ResponsiveContainer>
          </div>
      ...
      <div className="overflow-hidden rounded-2xl border border-gray-200/80 dark:border-gray-600/50 bg-white dark:bg-gray-800/90 shadow-lg">
        <div className="border-b border-gray-100 dark:border-gray-700/50 px-6 py-4">
          <h3 className="text-lg font-semibold text-gray-900 dark:text-white">Vue détaillée</h3>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-0.5">Dates et titres uniques</p>
        </div>
        <div className="overflow-x-auto">
          <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
            <thead className="bg-gray-50/80 dark:bg-gray-800/50">
              <tr>
                <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Rang</th>
                <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Artiste</th>
                <th scope="col" className="px-6 py-3 text-right text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Écoutes</th>
                <th scope="col" className="px-6 py-3 text-right text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Titres</th>
                <th scope="col" className="px-6 py-3 text-right text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Première</th>
                <th scope="col" className="px-6 py-3 text-right text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Dernière</th>
              </tr>
            </thead>
                ...
                return (
                  <tr key={artist.artistId} className="hover:bg-gray-50 dark:hover:bg-gray-700/30 transition-colors">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-right text-sm text-gray-500 dark:text-gray-400">
                      {new Date(artist.firstListenDate).toLocaleDateString("fr-FR")}
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-right text-sm text-gray-500 dark:text-gray-400">
                      {new Date(artist.lastListenDate).toLocaleDateString("fr-FR")}
                    </td>
                  </tr>
                );
              })}
    ...
    <div className="px-4 py-6 sm:px-0">
      <header className="mb-8">
        ...
        <p className="mt-2 text-base text-gray-500 dark:text-gray-400 max-w-2xl">
          Découvrez vos artistes préférés, leurs statistiques et l&apos;évolution de vos écoutes — inspiré d&apos;Apple Music Replay.
        </p>
      </header>
