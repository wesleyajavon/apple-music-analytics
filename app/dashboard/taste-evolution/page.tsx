...
function TrendCard({ trend }: { trend: WeekToWeekTrend }) {
  const classificationColor = CLASSIFICATION_COLORS[trend.classification];
  const classificationLabel = CLASSIFICATION_LABELS[trend.classification];
      ...
      <div className="px-5 py-4 border-b border-gray-100 dark:border-gray-700/50 flex flex-wrap items-center justify-between gap-3">
        <div>
          <h3 className="font-semibold text-gray-900 dark:text-white">
            {trend.timeRange.label} vs {trend.previousWeekRange.label}
          </h3>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-0.5">
            {trend.currentWeekListens} écoutes (vs {trend.previousWeekListens})
          </p>
        </div>
        <span
      ...
      <div className="space-y-6">
        <header className="mb-8">
          ...
          <p className="mt-2 text-base text-gray-500 dark:text-gray-400 max-w-2xl">
            Analyse des changements semaine à semaine...
          </p>
        </header>
  ...
  if (error) {
    return (
      <div className="space-y-6">
        ...
        <ErrorState
          error={error}
          message="Erreur lors du chargement de l'évolution des goûts"
          onRetry={handleRetry}
        />
      </div>
    );
    ...
    return (
      <div className="space-y-6">
        <header className="mb-8">
          ...
          <p className="mt-2 text-base text-gray-500 dark:text-gray-400 max-w-2xl">
            Analyse des changements de vos goûts musicaux semaine à semaine.
          </p>
        </header>
        <EmptyState
          {...emptyStatePresets.importData}
          message="Données insuffisantes"
          description="Importez vos données d'écoute et sélectionnez une plage d'au moins 2 semaines. Chaque semaine doit contenir au moins 10 écoutes pour une analyse fiable."
        />
      </div>
    );
  }
  ...
  return (
    <div className="space-y-8">
      <header>
        ...
        <p className="mt-2 text-base text-gray-500 dark:text-gray-400 max-w-2xl">
          Changements de vos goûts musicaux semaine à semaine. Les tendances
          sont calculées de manière déterministe ; l&apos;IA fournit une
      ...
      {data.skippedWeeks.length > 0 && (
        <div className="rounded-lg bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800/50 px-4 py-3">
          <p className="text-sm text-amber-800 dark:text-amber-200">
            <strong>Semaines exclues :</strong>{" "}
      ...
      <div className="space-y-6">
        <h2 className="text-xl font-semibold text-gray-900 dark:text-white">
          Tendances semaine à semaine
        </h2>
        <div className="space-y-6">
          {[...data.trends].reverse().map((trend) => (
            <TrendCard key={trend.timeRange.weekStart} trend={trend} />
          ))}
        </div>
      </div>
    </div>
