"use client";

import { Search } from "lucide-react";
import { useTranslations } from "next-intl";
import {
  DASHBOARD_BTN_GHOST,
  DASHBOARD_LIST_ROW,
  DASHBOARD_LIST_SEPARATOR,
  DASHBOARD_SEARCH_FIELD,
} from "@/lib/components/dashboard-ui";

export type GenreChartRow = {
  name: string;
  value: number;
  percentage: number;
  count: number;
  rank: number;
};

function GenresRankingSearchField({
  value,
  onChange,
  id,
}: {
  value: string;
  onChange: (value: string) => void;
  id: string;
}) {
  const t = useTranslations("genres");
  return (
    <div className="space-y-2">
      <label htmlFor={id} className="sr-only">
        {t("rankingSearchAria")}
      </label>
      <div className="relative">
        <Search
          className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted"
          aria-hidden
        />
        <input
          id={id}
          type="search"
          value={value}
          onChange={(event) => onChange(event.target.value)}
          placeholder={t("rankingSearchPlaceholder")}
          autoComplete="off"
          spellCheck={false}
          className={DASHBOARD_SEARCH_FIELD}
        />
      </div>
    </div>
  );
}

export function GenresRankingList({
  rows,
  page,
  pageSize,
  totalPages,
  total,
  offset,
  isFetching,
  onPageChange,
  onPageSizeChange,
  locale,
  searchInput,
  onSearchInputChange,
  searchFieldId,
  layout,
  onOpenGenre,
}: {
  rows: GenreChartRow[];
  page: number;
  pageSize: number;
  totalPages: number;
  total: number;
  offset: number;
  isFetching: boolean;
  onPageChange: (nextPage: number) => void;
  onPageSizeChange: (nextPageSize: number) => void;
  locale: string;
  searchInput: string;
  onSearchInputChange: (value: string) => void;
  searchFieldId: string;
  layout: "table" | "list";
  onOpenGenre?: (genre: GenreChartRow) => void;
}) {
  const t = useTranslations("genres");
  const tm = useTranslations("genres.mobile");
  const pageStart = total === 0 ? 0 : offset + 1;
  const pageEnd = Math.min(offset + rows.length, total);
  const hasMore = page < totalPages;

  return (
    <div className="w-full min-w-0">
      <div className="max-w-sm">
        <GenresRankingSearchField
          id={searchFieldId}
          value={searchInput}
          onChange={onSearchInputChange}
        />
      </div>
      {layout === "table" ? (
        <div className="mt-4 overflow-x-auto">
          <table className="min-w-full">
            <thead>
              <tr className={DASHBOARD_LIST_SEPARATOR}>
                <th scope="col" className="py-3 pr-4 text-left text-[13px] font-medium text-muted">
                  #
                </th>
                <th scope="col" className="py-3 pr-4 text-left text-[13px] font-medium text-muted">
                  {t("statGenres")}
                </th>
                <th scope="col" className="py-3 pr-4 text-right text-[13px] font-medium text-muted">
                  {t("listens")}
                </th>
                <th scope="col" className="py-3 text-right text-[13px] font-medium text-muted">
                  %
                </th>
              </tr>
            </thead>
            <tbody>
              {isFetching
                ? Array.from({ length: Math.min(pageSize, 10) }).map((_, index) => (
                    <tr key={`genre-rank-skeleton-${index}`} className={DASHBOARD_LIST_SEPARATOR}>
                      <td className="py-3 pr-4">
                        <div className="h-4 w-8 animate-pulse rounded bg-black/10 dark:bg-white/10" />
                      </td>
                      <td className="py-3 pr-4">
                        <div className="h-4 w-40 animate-pulse rounded bg-black/10 dark:bg-white/10" />
                      </td>
                      <td className="py-3 pr-4">
                        <div className="ml-auto h-4 w-12 animate-pulse rounded bg-black/10 dark:bg-white/10" />
                      </td>
                      <td className="py-3">
                        <div className="ml-auto h-4 w-10 animate-pulse rounded bg-black/10 dark:bg-white/10" />
                      </td>
                    </tr>
                  ))
                : rows.length === 0
                  ? (
                      <tr>
                        <td colSpan={4} className="py-10 text-center text-[13px] text-muted">
                          {t("rankingSearchEmpty")}
                        </td>
                      </tr>
                    )
                  : rows.map((genre) => (
                      <tr key={genre.name} className={DASHBOARD_LIST_SEPARATOR}>
                        <td className="whitespace-nowrap py-3 pr-4 text-[13px] tabular-nums text-muted">
                          {genre.rank}
                        </td>
                        <td className="py-3 pr-4 text-[13px] font-semibold text-foreground">{genre.name}</td>
                        <td className="whitespace-nowrap py-3 pr-4 text-right text-[13px] font-semibold tabular-nums text-foreground">
                          {genre.count.toLocaleString(locale)}
                        </td>
                        <td className="whitespace-nowrap py-3 text-right text-[13px] tabular-nums text-muted">
                          {genre.percentage.toFixed(1)}%
                        </td>
                      </tr>
                    ))}
            </tbody>
          </table>
        </div>
      ) : (
        <div className="mt-4">
          {isFetching
            ? Array.from({ length: Math.min(pageSize, 10) }).map((_, index) => (
                <div
                  key={`genre-rank-skeleton-${index}`}
                  className={`${DASHBOARD_LIST_ROW} ${DASHBOARD_LIST_SEPARATOR}`}
                  aria-hidden
                >
                  <div className="h-4 w-8 shrink-0 animate-pulse rounded bg-black/10 dark:bg-white/10" />
                  <div className="h-4 w-40 animate-pulse rounded bg-black/10 dark:bg-white/10" />
                  <div className="ml-auto h-4 w-12 animate-pulse rounded bg-black/10 dark:bg-white/10" />
                </div>
              ))
            : rows.length === 0
              ? (
                  <p className="py-10 text-center text-[13px] text-muted">{t("rankingSearchEmpty")}</p>
                )
              : rows.map((genre) => (
                  <button
                    key={genre.name}
                    type="button"
                    className={`${DASHBOARD_LIST_ROW} ${DASHBOARD_LIST_SEPARATOR} w-full cursor-pointer focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring`}
                    aria-label={tm("sheetOpenAria", { name: genre.name })}
                    onClick={() => onOpenGenre?.(genre)}
                  >
                    <span className="w-8 shrink-0 text-[13px] font-semibold tabular-nums text-muted">
                      {genre.rank}
                    </span>
                    <span className="min-w-0 flex-1 truncate text-[13px] font-semibold text-foreground">
                      {genre.name}
                    </span>
                    <span className="shrink-0 text-[13px] font-semibold tabular-nums text-foreground">
                      {genre.count.toLocaleString(locale)}
                    </span>
                    <span className="w-12 shrink-0 text-right text-[13px] tabular-nums text-muted">
                      {genre.percentage.toFixed(1)}%
                    </span>
                  </button>
                ))}
        </div>
      )}
      <div className="mt-6 flex flex-col gap-3 sm:flex-row sm:flex-wrap sm:items-center sm:justify-between">
        <p className="text-[13px] text-muted">
          {t("paginationSummary", {
            start: pageStart,
            end: pageEnd,
            total,
          })}
        </p>
        <div className="flex flex-wrap items-center gap-2">
          <button
            type="button"
            onClick={() => onPageChange(page - 1)}
            disabled={page === 1}
            className={DASHBOARD_BTN_GHOST}
          >
            {t("paginationPrevious")}
          </button>
          <label className="inline-flex min-h-11 items-center gap-2 text-[13px] text-muted">
            <span>{t("pageSizeLabel")}</span>
            <select
              value={pageSize}
              onChange={(event) => onPageSizeChange(Number(event.target.value))}
              className="h-11 rounded-full border border-glass-hairline bg-surface-raised px-3 text-[13px] text-foreground"
            >
              <option value={10}>10</option>
              <option value={20}>20</option>
              <option value={50}>50</option>
            </select>
          </label>
          <span className="px-2 text-[13px] text-muted">
            {t("paginationPage", { page, totalPages })}
          </span>
          <button
            type="button"
            onClick={() => onPageChange(page + 1)}
            disabled={!hasMore}
            className={DASHBOARD_BTN_GHOST}
          >
            {t("paginationNext")}
          </button>
        </div>
      </div>
    </div>
  );
}
