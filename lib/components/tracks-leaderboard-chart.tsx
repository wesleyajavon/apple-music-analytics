...
export type TracksBarPoint = {
  name: string;
  fullName: string;
  listens: number;
};

export function TracksLeaderboardChart({
  data,
  locale,
}: {
  data: TracksBarPoint[];
      ...
      >
        ...
        <Tooltip
          ...
          formatter={(value: number, name: string, props: { payload?: { fullName?: string } }) => {
            ...
            if (name === "listens") {
              return [`${value.toLocaleString(locale)} ${t("listensCount")}`, fullName || t("track")];
            }
            return [value, name];
          }}
        />
...
export function TracksLeaderboardChartSkeleton() {
  return (
